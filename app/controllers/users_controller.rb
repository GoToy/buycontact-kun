       ...
       when /change_contact_num/
         ...
         user.update(remain: before_update_num - parsed_data['num'][0].to_i)
          if user.reload.remain == 8
            message = {
              type: 'text',
              text: "残数#{before_update_num}個に対し、#{parsed_data['num'][0].to_i}個引いて、#{user.reload.remain}個になりました\nhttps://www.lensmode.com/auth/login/redirectUrl/%252Fmypage%252Findex%252F/"
            }
          else 
            message = {
              type: 'text',
          ...
          if parsed_message.present?
           ...
           if parsed_message > 0
            ...
            if user.reload.remain == 7
              message = {
                type: 'text',
                text: "残数#{before_update_num}個に対し、#{parsed_message.abs.to_i}個引いて、#{user.reload.remain}個になりました\nhttps://www.lensmode.com/auth/login/redirectUrl/%252Fmypage%252Findex%252F/"
              }
            else 
              message = {
                type: 'text',
